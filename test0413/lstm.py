...
class PTBModel(object):
    ...
    def __init__(self, is_training, py_config):
        """
        :param is_training: 是否要进行训练.如果is_training=False,则不会进行参数的修正。
        """
        self.batch_size = batch_size = py_config.batch_size
        self.num_steps = num_steps = py_config.num_steps
        size = py_config.hidden_size
        vocab_size = py_config.vocab_size
        ...
        if is_training and py_config.keep_prob < 1:
            inputs = tf.nn.dropout(inputs, py_config.keep_prob)
...
def run_epoch(t_session, model, data, eval_op, verbose=False):
    ...
    for step, (x, y) in enumerate(reader.ptb_iterator(data, model.batch_size,
        ...
        feed_dict = {model.input_data: x, model.targets: y}  # 设定input和target的值
...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        with tf.variable_scope("model", reuse=None, initializer=initializer):
            m = PTBModel(is_training=True, py_config=config)  # 训练模型， is_trainable=True
        with tf.variable_scope("model", reuse=True, initializer=initializer):
            mvalid = PTBModel(is_training=False, py_config=config)  # 交叉检验和测试模型，is_trainable=False
            mtest = PTBModel(is_training=False, py_config=eval_config)
